// RP Archiver About System
// Manages the how-to guide modal

class RPArchiverAbout {
    constructor() {
        this.aboutData = null;
        this.modalId = 'rp-archiver-about-modal';
    ...
    async loadAboutData() {
        try {
            const response = await fetch('about/rp-archiver-about.json');
            if (!response.ok) {
                throw new Error('Failed to load about data');
            }
            this.aboutData = await response.json();
        } catch (error) {
            console.error('Error loading RP Archiver about data:', error);
            // Fallback data
            this.aboutData = {
                title: "RP Archiver Guide",
                sections: [
                    {
                        id: "error",
                        title: "Error",
    ...
    setupEventListeners() {
        // About button click
        const aboutBtn = document.getElementById('rp-archiver-about-btn');
        if (aboutBtn) {
    ...
    createModal() {
        ...
        modal.className = 'modal-overlay rp-archiver-about-overlay';
    ...
    getModalHTML() {
        ...
        return `
            <div class="modal-content rp-archiver-about-content" tabindex="-1">
                <div class="modal-header">
                    <h2>${this.aboutData.title}</h2>
                    <button class="close-btn" id="rp-archiver-about-close" title="Close">
    ...
    attachModalEventListeners() {
        ...
        // Close button
        const closeBtn = modal.querySelector('#rp-archiver-about-close');
        if (closeBtn) {
...
// Initialize the about system
let rpArchiverAbout = null;

// Export for global use
window.initializeRPArchiverAbout = async function() {
    if (!rpArchiverAbout) {
        rpArchiverAbout = new RPArchiverAbout();
        await rpArchiverAbout.initialize();
    }
    return rpArchiverAbout;
};