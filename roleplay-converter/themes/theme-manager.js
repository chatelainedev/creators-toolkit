...
class RPThemeManager {
    ...
    async loadThemes() {
        try {
            // Adjust path as needed - if sharing themes.json
            const response = await fetch('../themes/themes.json');
            // OR if you copied themes.json to RP Archiver folder:
            // const response = await fetch('themes/themes.json');
            
            this.themes = await response.json();
        } catch (error) {
            console.error('Failed to load themes in RP Archiver:', error);
    ...
    applyTheme(themeId) {
        ...
        console.log(`RP Archiver theme applied: ${theme.name}`);
    }
    ...
    setupStorageListener() {
        window.addEventListener('storage', (e) => {
            if (e.key === this.SHARED_THEME_KEY && e.newValue !== e.oldValue) {
                console.log('Theme changed in main app, updating RP Archiver...');
                this.loadSharedTheme();
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.rpThemeManager = new RPThemeManager();
});