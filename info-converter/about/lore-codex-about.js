// Lore Codex About System
// Manages the how-to guide modal

class LoreCodexAbout {
    constructor() {
        this.aboutData = null;
        this.modalId = 'lore-codex-about-modal';
    ...
    async loadAboutData() {
        try {
            const response = await fetch('about/lore-codex-about.json');
            if (!response.ok) {
                throw new Error('Failed to load about data');
            }
            this.aboutData = await response.json();
        } catch (error) {
            console.error('Error loading Lore Codex about data:', error);
            // Fallback data
            this.aboutData = {
                title: "Lore Codex Guide",
                sections: [
                    {
                        id: "error",
                        title: "Error",
    ...
    setupEventListeners() {
        // About button click
        const aboutBtn = document.getElementById('lore-codex-about-btn');
        if (aboutBtn) {
    ...
    createModal() {
        ...
        modal.className = 'modal-overlay lore-codex-about-overlay';
    ...
    getModalHTML() {
        ...
        return `
            <div class="modal-content lore-codex-about-content" tabindex="-1">
                <div class="modal-header">
                    <h2>${this.aboutData.title}</h2>
                    <button class="close-btn" id="lore-codex-about-close" title="Close">
    ...
    attachModalEventListeners() {
        ...
        // Close button
        const closeBtn = modal.querySelector('#lore-codex-about-close');
        if (closeBtn) {
...
// Initialize the about system
let loreCodexAbout = null;

// Export for global use
window.initializeLoreCodexAbout = async function() {
    if (!loreCodexAbout) {
        loreCodexAbout = new LoreCodexAbout();
        await loreCodexAbout.initialize();
    }
    return loreCodexAbout;
};